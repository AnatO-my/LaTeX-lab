...
\begin{quizsolution}
  For a spring oscillator,
    ...
    \[
      ...
      =\frac{E}{4}.
    \]
    Energy conservation gives \(K=E-U=3E/4\). Hence option \textbf{B}.
\end{quizsolution}
...
\begin{quizsolution}
  Out-of-phase motion requires equal and opposite displacements:
    ...
    I may write vectors like \({[1, 1]}^T\), instead of the usual \({\displaystyle{\binom{1}{1}}}\) Therefore, option \textbf{D}.
\end{quizsolution}
...
\end{quizquestion}
\begin{quizsolution}
  When the masses move oppositely, their relative displacement is large. The middle spring is therefore stretched or compressed substantially, so it contributes an additional restoring force. A stronger effective restoring action produces a larger angular frequency. Hence option \textbf{C}.
\end{quizsolution}
...
\begin{quizsolution}
  Snell's law gives
    ...
    \]
    Therefore, \(\theta_2=\sin^{-1}(1/3)=19.5^\circ\), so option \textbf{D}.
\end{quizsolution}
...
\begin{quizsolution}
  For the first single-slit minimum,
    ...
    \]
    Thus, \(\theta\approx2.50\times10^{-3}\ \mathrm{rad}=0.143^\circ\). Option \textbf{E}.
\end{quizsolution}
...
\begin{quizsolution}
  During the short collision, horizontal momentum is conserved:
    ...
    \]
    Starting at equilibrium with speed \(V\), the amplitude is \(A=V/\omega=2.00/20.0=\SI{0.100}{m}\). Option \textbf{A}.
\end{quizsolution}
...
\begin{quizsolution}
  Direct substitution gives
    ...
    \]
    Thus, option \textbf{A}. The positive \(q_2\) indicates a substantial out-of-phase component.
\end{quizsolution}
...
\begin{quizsolution}
  For a closed-open pipe,
    ...
    \]
    Only odd harmonics occur, so the next two are \(3f_1=\SI{300}{Hz}\) and \(5f_1=\SI{500}{Hz}\). Therefore, option \textbf{C}.
\end{quizsolution}
...
\begin{quizsolution}
  The increased tension raises the frequency to \(505\ \mathrm{Hz}\). For the same string length and linear density,
    ...
    \]
    The fractional increase is \(1.0201-1=0.0201\). Hence option \textbf{D}.
\end{quizsolution}
...
\begin{quizsolution}
  For a moving source approaching a stationary observer,
    ...
    \]
    The observed frequency is higher because the source approaches. Option \textbf{A}.
\end{quizsolution}
...
\begin{quizsolution}
  For critical incidence from glass to air,
    ...
    \]
    so \(\theta_B=56.3^\circ\). Therefore, option \textbf{A}.
\end{quizsolution}
...
\begin{quizsolution}
  Write \(x=A\cos(\omega t+\phi)\). At \(t=0\), \(x=A/2\), so \(\cos\phi=1/2\). Moving toward equilibrium from the positive side means \(v<0\), hence \(\sin\phi>0\) and \(\phi=\pi/3\). The opposite turning point occurs when the phase first reaches \(\pi\):
    \[
      ...
      \omega t=\frac{2\pi}{3}.
    \]
    Since \(T=2\pi/\omega\), \(t=T/3\). Thus, option \textbf{A}.
\end{quizsolution}
...
\begin{quizsolution}
  During sticking, horizontal momentum is conserved because the clay initially has no horizontal momentum:
    ...
    \]
    At equilibrium all post-collision oscillator energy is kinetic, so \(A'=V/\omega'=1.60/8.94=\SI{0.179}{m}\). Option \textbf{E}.
\end{quizsolution}
...
\begin{quizsolution}
  For the initial condition \({[A,0]}^T\), the displacement of the first mass can be written
    ...
    \]
    At this time the amplitude has transferred to the other mass. Option \textbf{D}.
\end{quizsolution}
...
\begin{quizsolution}
  Acting with \(K\) on the mode vectors gives
    ...
    \]
    so \(C_1=2\ \mathrm{cm}\) and \(C_2=1\ \mathrm{cm}\). Thus, option \textbf{E}.
\end{quizsolution}
...
\begin{quizsolution}
  The total time is the fall time plus the upward sound-travel time:
    ...
    \[
      h=u^2=\SI{70.5}{m}.
    \]
    As a check, the fall time is \SI{3.794}{s} and the sound time is \SI{0.206}{s}, summing to \SI{4.00}{s}. Hence option \textbf{B}.
\end{quizsolution}
...
\begin{quizsolution}
  Far from the sources, the path difference is \(\Delta L=d\cos\theta\) (an equivalent sine convention gives the same count). Maxima require
    ...
    \[
      m=-3,-2,-1,0,1,2,3,
    \]
    seven values. For each value, \(|m\lambda/d|<1\), so \(\cos\theta=m\lambda/d\) has two positions during a full circuit. Therefore, the total is \(7\times2=14\). Option \textbf{D}.
\end{quizsolution}
...
\begin{quizsolution}
  The parameter is
    ...
    \[
      ...
      =4.29\times10^{-3}.
    \]
    Hence \(\theta\approx0.246^\circ\). The first secondary maximum has the standard intensity \(I\approx0.047I_{\max}\), or \(4.7\%\). Therefore, option \textbf{D}.
\end{quizsolution}
